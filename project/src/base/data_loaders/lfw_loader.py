...
class LFWDL(DataLoader):
    def __init__(self, aligned):
        super().__init__(DLName.LFW, aligned, f'{cts.BASE_PATH}/lfw', False)
        self.set_dirs_paths()
    ...
    def set_dirs_paths(self):
        path = None
        if self.aligned:
            path = os.path.join(self.dataset_path,'aligned_lfw')
        else:
            path = os.path.join(self.dataset_path,'lfw')
